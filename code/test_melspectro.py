...
path = os.path.join(os.getcwd(), 'audio_test', 'car_horn.wav')
...
plt.title('1. Spectrogram Standar (Sumbu Y: Hertz)')

# Plot 2: Mel-Spectrogram (Skala Mel)
plt.subplot(3, 1, 2)
librosa.display.specshow(S, sr=sr, x_axis='time', y_axis='mel')
plt.colorbar(format='%+2.0f')
plt.title('2. Mel-Spectrogram (Sumbu Y: Mel)')

# Plot 3: Log-Mel Spectrogram (Mel + Desibel)
plt.subplot(3, 1, 3)
librosa.display.specshow(S_db, sr=sr, x_axis='time', y_axis='mel', fmax=8000)
plt.colorbar(format='%+2.0f dB')
plt.title('3. Log-Mel Spectrogram (Mel + Log)')
