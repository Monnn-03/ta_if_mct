...
\begin{enumerate}
    ...
    Langkah ini diterapkan karena terdapat beberapa data audio pada Dataset UrbanSound8K dengan format stereo (dua saluran). 
    Untuk menyeragamkan dimensi input pada model nanti, seluruh data audio dikonversi menjadi format mono (satu saluran).
...
Konsep utama dari \textit{Transfer Learning} adalah memanfaatkan pengetahuan yang telah dipelajari oleh model terlatih sebelumnya (\textit{pre-trained model}) dengan dataset berskala besar untuk diterapkan pada permasalahan spesifik yang memiliki keterbatasan data.
Dalam penelitian ini, arsitektur PANNs yang digunakan merupakan \textit{pre-trained model} yang sebelumnya telah dilatih menggunakan dataset AudioSet \cite{kong2020panns}.
AudioSet merupakan sebuah dataset audio berskala besar yang berisi lebih dari 2.084.320 klip suara berdurasi 10 detik yang mencakup berbagai macam suara manusia, hewan, alat musik, genre dan suara lingkungan dengan 632 kategori kelas (\textit{multi-label classification}) \cite{gemmeke2017audioset}.
...
Pada fase \textit{pre-training} tersebut, para peneliti PANNs melatih beberapa varian arsitekturnya menggunakan representasi input yang spesifik, yakni \textit{Raw Waveform}, \textit{Log-mel Spectrogram}, serta kombinasi keduanya (\textit{Hybrid}), untuk mengenali berbagai pola hierarkis suara \cite{kong2020panns}.
...
Setelah model dikonfigurasi sesuai kebutuhan penelitian, tahap selanjutnya adalah melakukan \textit{fine-tuning model} secara menyeluruh pada dataset UrbanSound8K.
...
Hal ini bertujuan agar fitur ekstraktor pada model PANNs dapat beradaptasi sepenuhnya terhadap karakteristik fitur yang diambil dari audio dataset UrbanSound8K, khususnya pada 4 kelas yang telah diseleksi. \par
...
\begin{enumerate}
    ...
    Dikarenakan mekanisme pengujian menggunakan \textit{5-fold cross validation}, maka hasil akhir \textit{Macro F1-Score} pada setiap model dengan representasi input tertentu akan dilaporkan dalam format rata-rata beserta rentang simpangannya (Rata-rata $\pm$ Standar Deviasi).
...
Selain menggunakan ketiga indikator tersebut, penelitian ini juga menerapkan uji statistik formal untuk membandingkan performa akhir dari setiap model dengan ketiga representasi input (\textit{Raw Waveform}, \textit{Log-mel Spectrogram}, dan \textit{Hybrid}).
