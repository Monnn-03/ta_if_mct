...
Tahap awal penelitian dimulai dengan mengidentifikasi urgensi pengembangan sistem alat bantu bagi penyandang tunarungu yang berorientasi pada standar keselamatan tinggi (\textit{safety-critical}). Alat bantu ini diharapkan tidak hanya mengamplifikasi sinyal, tetapi juga mampu mengenali suara bahaya di lingkungan sekitar secara otomatis.
Tantangan teknis utama yang dihadapi adalah keterbatasan jumlah sampel pada dataset target (\textit{limited dataset}) yang spesifik untuk suara bahaya di lingkungan. Melatih model \textit{Deep Learning} dari awal (\textit{from scratch}) dengan data terbatas berisiko tinggi menyebabkan \textit{overfitting} dan performa yang buruk. Oleh karena itu, penggunaan arsitektur \textit{Pre-trained Audio Neural Networks} (PANNs) yang telah dilatih pada dataset raksasa AudioSet menjadi solusi strategis (\textit{Transfer Learning}) untuk mengatasi kelangkaan data tersebut.
...
Tahap ini dilakukan untuk membangun landasan teoretis yang kokoh sebelum perancangan sistem dimulai. Studi literatur difokuskan pada analisis mendalam terhadap arsitektur \textit{Pre-trained Audio Neural Networks} (PANNs), khususnya varian CNN14, serta mekanisme \textit{Transfer Learning} untuk adaptasi domain. 
...
Data penelitian bersumber dari dataset publik \textit{UrbanSound8K} yang memuat 8732 potongan audio (\textit{slices}) berdurasi kurang dari 4 detik. Tahap pra-pemrosesan dilakukan melalui tiga langkah strategis untuk menjamin integritas data:
...
Hasil akhir dari proses ini disimpan dalam format konfigurasi statis (\texttt{split.json}) yang menjadi acuan baku bagi seluruh skenario pengujian model. \par

\subsection{Perancangan dan Adaptasi Model} \label{III.Langkah 4}
Penelitian ini menggunakan arsitektur dasar \textit{Cnn14} dari pustaka PANNs yang telah dilatih sebelumnya (\textit{pre-trained}) pada dataset AudioSet. Untuk mengadaptasi model tersebut ke dalam domain permasalahan yang baru, dilakukan serangkaian modifikasi struktural sebagai berikut:

\begin{enumerate}
    \item \textbf{Modifikasi Lapisan Luaran (\textit{Output Layer}):} 
    Lapisan klasifikasi asli yang memiliki 527 \textit{node} (sesuai kelas AudioSet) digantikan dengan lapisan \textit{Fully Connected} baru yang hanya memiliki 4 \textit{node}. Lapisan ini merepresentasikan probabilitas kelas target: klakson, sirine, tembakan, dan gonggongan. Bobot pada lapisan awal dipertahankan dari model \textit{pre-trained} (\textit{Transfer Learning}), sedangkan lapisan baru diinisialisasi ulang untuk dipelajari selama proses pelatihan.

    \item \textbf{Perancangan Skenario Input:} 
    Untuk menjawab rumusan masalah mengenai representasi fitur terbaik, dirancang tiga skenario variasi input yang akan diaplikasikan secara terpisah pada arsitektur model:
    \begin{itemize}
        \item \textbf{Skenario 1 (1D):} Menggunakan input \textit{Raw Waveform} (gelombang amplitudo terhadap waktu) untuk mengekstraksi fitur langsung dari domain waktu.
        \item \textbf{Skenario 2 (2D):} Menggunakan input \textit{Log-mel Spectrogram} yang diekstraksi secara \textit{on-the-fly} untuk menangkap pola representasi frekuensi-waktu.
        \item \textbf{Skenario 3 (Hybrid):} Menggunakan arsitektur \textit{Wavegram-Logmel} yang menggabungkan fitur domain waktu dan frekuensi secara simultan.
    \end{itemize}
\end{enumerate}

Ketiga variasi model ini akan dilatih menggunakan konfigurasi dan data yang identik untuk memastikan validitas hasil komparasi. \par

\subsection{Eksperimen dan Evaluasi Model} \label{III.Langkah 5}
Tahap akhir melibatkan pelaksanaan pelatihan dan pengujian model secara sistematis menggunakan skema validasi silang 5 lipatan (\textit{5-Fold Cross-Validation}). Setiap skenario input (1D, 2D, dan Hybrid) akan dilatih sebanyak 5 putaran, di mana pada setiap putaran model menggunakan kombinasi \textit{fold} latih dan \textit{fold} uji yang berbeda sesuai konfigurasi yang telah ditetapkan pada tahap pra-pemrosesan.
...
\subsection{Analisis Hasil dan Penarikan Kesimpulan} \label{III.Langkah 6}
...
Berdasarkan analisis tersebut, ditarik kesimpulan akhir mengenai model mana yang memiliki performa paling optimal. Kesimpulan ini tidak hanya menjawab rumusan masalah secara statistik, tetapi juga memberikan rekomendasi teknis mengenai kelayakan implementasi model terpilih pada sistem alat bantu keselamatan bagi penyandang tunarungu. \par
