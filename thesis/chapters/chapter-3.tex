...
\begin{enumerate}
    ...
    Performa model hasil \textit{Fine-tuning} dievaluasi menggunakan metrik \textit{F1-Score}, \textit{Loss} dan \textit{Confusion Matrix}, serta analisis efisiensi waktu komputasi untuk memastikan kelayakan implementasi secara \textit{real-time}.
...
Jumlah \textit{neuron} pada lapisan akhir disesuaikan dari 527 \textit{node} menjadi 4 \textit{node} (sesuai jumlah kelas pilihan). \par
...
Akan tetapi, klasifikasi model yang ditentukan pada penelitian ini adalah klasifikasi \textit{Single-Label}, sehingga terjadi modifikasi pada lapisan akhir menjadi lapisan \textit{Linear} (Logits) yang memilih satu kelas dengan probabilitas paling tinggi daripada kelas lainnya. \par
...
\section{Konfigurasi Parameter Pelatihan} \label{sec:konfigurasi_eksperimen}
