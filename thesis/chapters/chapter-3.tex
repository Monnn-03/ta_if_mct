...
\begin{enumerate}
    ...
    Ketersediaan data menjadi kunci utama pelatihan model. Dataset sekunder \textit{UrbanSound8K} dipilih karena memuat data lingkungan perkotaan yang relevan. Dari dataset ini, dilakukan seleksi kelas suara yang merepresentasikan sinyal ancaman bahaya. Kendala keterbatasan jumlah sampel pada kelas tertentu menjadi tantangan yang diantisipasi sejak awal.
    ...
    Tahap ini mempelajari karakteristik dataset dan model:
    \begin{itemize}
        \item \textbf{Dataset:} \textit{UrbanSound8K} memiliki struktur 10 \textit{fold} bawaan. Dokumentasi memperingatkan agar tidak mencampur data antar \textit{fold} secara sembarangan untuk mencegah kebocoran data (\textit{data leakage}) yang dapat menyebabkan \textit{overfitting} (model hanya menghafal data latih namun memiliki generalisasi buruk pada data baru).
        \item \textbf{Model:} PANNs menyediakan berbagai variasi arsitektur. Dipilih tiga model representatif: \textit{Res1dNet31} (Raw Waveform), \textit{ResNet38} (Log-mel Spectrogram), dan \textit{Hybrid} (gabungan keduanya) untuk dikomparasi kinerjanya.
    \end{itemize}

    \item \textbf{Pra-pemrosesan Dataset (\textit{Pre-processing})} \\
    Serangkaian proses dilakukan untuk mengubah data mentah menjadi format yang siap latih, meliputi penanganan struktur \textit{fold}, seleksi kelas, dan penyesuaian format audio.
...
\section{Akuisisi Data} \label{III.Akuisisi}
Sumber data utama dalam penelitian ini adalah dataset publik \textbf{UrbanSound8K}. Dataset diunduh secara manual dari repositori Kaggle dalam format terkompresi (\texttt{.zip}). Setelah diekstraksi, struktur dataset terdiri dari 10 folder (masing-masing mewakili satu \textit{fold}) beserta satu file metadata (\texttt{.csv}) yang memuat informasi nama file audio, \textit{class ID}, dan \textit{fold} asal.

\section{Pra-pemrosesan Data} \label{III.Preprocessing}
Tahap pra-pemrosesan dilakukan untuk menjamin integritas data dan mencegah kebocoran informasi (\textit{data leakage}) selama proses pelatihan.
