...
Setelah model dikonfigurasi sesuai kebutuhan penelitian, tahap selanjutnya adalah melakukan eksperimen \textit{Transfer Learning} dengan model PANN.
...
\subsection{Reproduksibilitas Eksperimen (\textit{Random Seed})}
Dalam melatih model \textit{Deep Learning}, terdapat banyak proses yang melibatkan elemen acak (seperti inisialisasi bobot, pengacakan *batch* data, dan \textit{random cropping}). Untuk memastikan bahwa eksperimen ini bersifat deterministik dan dapat direproduksi ulang (\textit{reproducible}) dengan hasil yang sama persis, seluruh elemen pengacakan pada modul komputasi (PyTorch dan NumPy) dikunci menggunakan nilai \textit{Random Seed} sebesar 42.
...
Proses pembaruan bobot model (\textit{weight update}) dilakukan menggunakan algoritma optimasi \textbf{AdamW} (\textit{Adam with Weight Decay}). Pemilihan AdamW didasari oleh kemampuannya dalam memisahkan mekanisme pembaruan bobot dengan \textit{weight decay} (regularisasi L2), sehingga lebih efektif dalam mencegah \textit{overfitting} dibandingkan algoritma Adam standar. Pelatihan dilakukan dalam \textit{batch size} sebesar 8 selama maksimal 50 \textit{epoch}, dengan pengaturan awal \textit{Learning Rate} sebesar 5e-4 dan nilai \textit{Weight Decay} sebesar 1e-4.

Untuk mengoptimalkan proses konvergensi, diterapkan pula penyesuaian laju pembelajaran dinamis menggunakan \textbf{\textit{ReduceLROnPlateau}}. Mekanisme ini dirancang untuk memantau metrik \textit{Validation Loss}. Apabila nilai kerugian validasi tersebut tidak mengalami penurunan selama 3 \textit{epoch} berturut-turut (\textit{patience}), maka nilai \textit{Learning Rate} akan direduksi secara otomatis sebesar setengah dari nilai sebelumnya (faktor 0,5). Strategi ini membantu model untuk menemukan titik minimum global dengan lebih presisi saat mendekati akhir pelatihan.
...
\begin{enumerate}
    \item \textbf{\textit{Early Stopping}}: Pelatihan model akan dihentikan secara paksa lebih awal apabila metrik evaluasi tidak mencatatkan rekor performa baru selama 5 \textit{epoch} berturut-turut. Ini mencegah model menghafal data latih secara berlebihan.
    \item \textbf{\textit{Model Checkpointing}}: Selama proses pelatihan berjalan, sistem secara dinamis hanya akan menyimpan (\textit{save}) parameter bobot model pada \textit{epoch} yang berhasil mencetak nilai \textit{F1-Score} validasi tertinggi (\textit{best model}). Jika performa pada \textit{epoch} selanjutnya memburuk, model tidak akan disimpan, sehingga hasil akhir yang dievaluasi dipastikan adalah versi model yang paling optimal.
\end{enumerate}

\subsection{Pemantauan dan Pencatatan Eksperimen}
Seluruh pergerakan metrik evaluasi pelatihan (termasuk \textit{Train/Validation Loss}, \textit{F1-Score}, dan pergerakan \textit{Learning Rate}) direkam dan divisualisasikan secara \textit{real-time} menggunakan dasbor \textbf{Weights \& Biases (WandB)}. Penggunaan pustaka ini memastikan transparansi rekam jejak pelatihan. Di akhir setiap pengujian \textit{fold}, sistem juga secara otomatis menggenerasi laporan grafik riwayat pelatihan dan visualisasi \textit{Confusion Matrix} berdasarkan prediksi dari model terbaik (\textit{best checkpoint}) untuk keperluan analisis lanjutan.

\subsection{Konfigurasi Parameter Pelatihan} \label{sec:konfigurasi_eksperimen}
Sebelum melakukan uji ketiga model, terdapat beberapa konfigurasi parameter yang perlu diterapkan pada training model.
Hal ini bertujuan untuk menemukan konfigurasi pelatihan model yang paling optimal.
Rincian parameter konfigurasi yang dikendalikan dalam eksperimen ini disajikan pada Tabel \ref{tab:parameter_latih}. \par
...
Num Workers & 2 \\ \hline
Learning Rate & 0.0005 ($5e^{-4}$) \\ \hline
