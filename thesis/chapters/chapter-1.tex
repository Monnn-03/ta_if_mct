...
Pendekatan ini memanfaatkan \textit{Pre-trained Audio Neural Networks} (PANNs) \cite{kong2020panns}, yaitu sebuah kerangka kerja model \textit{Deep Learning} skala besar yang telah dilatih sebelumnya (\textit{pre-trained}) pada dataset AudioSet \cite{gemmeke2017audioset} untuk mengenali berbagai pola suara umum. 
...
Perbedaan karakteristik ukuran data ini memunculkan dugaan bahwa kompleksitas arsitektur model \textit{Hybrid} dan \textit{Log-mel Spectrogram} justru memiliki risiko \textit{overfitting} yang lebih tinggi dibandingkan model \textit{Raw Waveform} ketika dilatih pada dataset yang kecil.
...
Selain ketidakpastian tersebut, terdapat juga perbedaan mendasar pada jenis keluaran klasifikasi. 
Arsitektur PANNs dilatih untuk mendeteksi banyak label sekaligus (\textit{multi-label}), sedangkan penelitian ini dirancang untuk memprioritaskan identifikasi sumber bahaya paling dominan (\textit{single-label}).
Pendekatan \textit{single-label} ini dipilih untuk menyelaraskan karakteristik dataset UrbanSound8K yang memiliki anotasi satu sumber suara dominan (\textit{salient event}).
Meskipun demikian, penelitian ini menyadari bahwa rekaman suara pada dataset tersebut maupun di lingkungan perkotaan nyata tidak menjamin kondisi \textit{salient event}, sehingga tumpang tindih (\textit{overlapping}) antar suara sangat mungkin terjadi. 
...
Oleh karena itu, penelitian ini bertujuan utama secara teknis untuk menginvestigasi dan membandingkan kinerja tiga pendekatan representasi input, yaitu \textit{Raw Waveform}, \textit{Log-mel Spectrogram}, dan \textit{Hybrid} dalam mengklasifikasikan suara tanda bahaya yang mengancam keselamatan penyandang tunarungu. 
...
\begin{enumerate}[noitemsep]
  ...
  \item Representasi input manakah yang menghasilkan performa model paling optimal berdasarkan analisis \textit{Confusion Matrix}, metrik \textit{F1-Score}, dan kurva \textit{loss} untuk meminimalisir kesalahan deteksi pada sistem keselamatan penyandang tunarungu?
\end{enumerate}
...
\begin{enumerate}[noitemsep]
  ...
  \item Mengevaluasi pendekatan representasi input yang menghasilkan performa model paling optimal berdasarkan analisis \textit{Confusion Matrix}, metrik \textit{F1-Score}, dan kurva \textit{loss} untuk meminimalisir kesalahan deteksi pada sistem keselamatan penyandang tunarungu.
\end{enumerate}
