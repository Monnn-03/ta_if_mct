...
\usepackage[paperwidth=155mm, % Setting layout kertas UNESCO (155 x 230 mm)
			...
			bindingoffset=0.5cm]
			{geometry}
\usepackage{tabularx} % Untuk tabel dengan lebar tertentu
\usepackage{needspace} % Untuk mencegah pemisahan itemize/enumerate di halaman
\usepackage{microtype} % Mengatur hbox badness
\usepackage{array}
\input{config/config.sty} % Import konfigurasi laporan
\bibliography{references.bib} % Import list Daftar Pustaka

\begin{document}
    \setlength{\abovedisplayskip}{2pt plus 1pt minus 9pt}
    ...
    \title{Analisis Performa Arsitektur PANN\lowercase{s} Dengan Pendekatan Representasi Fitur Berbasis \textit{Raw Waveform}, \textit{Log-mel Spectrogram}, dan \textit{Hybrid} untuk Klasifikasi Suara Kedaruratan Perkotaan}
    % Judul Tugas Akhir dalam Bahasa Inggris
    \titleEN{Performance Analysis of PANN\lowercase{s} Architecture using Raw Waveform, Log-mel Spectrogram, and Hybrid Feature Representation for Urban Emergency Sound Classification}
    % JUDUL AKAN DITULIS DALAM HURUF KAPITAL; Font size 16 pt; Bold; Tidak melebihi 4 baris
    
    % Informasi Mahasiswa
	\author{Ramon Riping}		% Nama Mahasiswa
	\nim{122140078}				% NIM Mahasiswa
